...
Change data flows through the hub (HVR model), not peer-to-peer. Rationale: in zoned networks — defense enclaves, segmented manufacturing — source and target zones frequently cannot communicate directly, but both may talk to a broker host in a management zone. Hub routing means one host to accredit and two firewall paths per stream instead of an N×M mesh of change requests. Peer-to-peer transfer is preserved as a future routing *policy* (the file log format and ack protocol are identical either way), deferred, not designed out.

### 2.2 The console — application architecture

The web console is a **pure client of the public REST API** (section 7). It holds no privileged path, no private endpoint, and no server-rendered state; everything below is a view over API responses. This section is the map of that application — what the screens are, what they share, and the rules that keep them consistent.

#### 2.2.1 Four levels, one hierarchy

Every screen sits at exactly one level, and the level determines both scope and authority:

| Level | Object | Console screen | Can operate? |
|---|---|---|---|
| **Global Fleet** | The organization | Global Fleet console | No — read-only across systems |
| **Fleet** | One enrolled hub server | Fleet page | Yes, within the fleet |
| **Virtual Fleet** | One logical hub | VF page — **the working context** | Yes, within the VF |
| **Stream** | One replication unit | Stream detail | Yes, within the stream |

The Global Fleet console grants **sight, not touch**: operating on anything requires switching into the owning fleet or VF, which becomes the working context for every downstream screen at once — header selector, sidebar tree, and page content move together.

Beneath the VF sit its four peer surfaces — **Connections**, **Jobs**, **Events**, **VF Timeline** — and beneath each stream its own detail, tables, and timeline. Connections belong to the VF, not to a stream: an endpoint outlives the streams that use it.

As the navigation tree renders it:
...
├── Fleet · TITAN_1                          ★
│   │
│   ├── Virtual Fleet · orca-east
│   │   │
│   │   ├── Connections
│   │   │   ├── ↑ DB2-MFG                    source
│   │   │   ├── ↑ ORA-ERP01                  source
│   │   │   ├── ↑ PG-BILLING                 source
│   │   │   ├── ↓ DATABRICKS-LH              target
│   │   │   ├── ↓ PG-ANALYTICS               target
...
│   │   └── Fleet Alerts                     2
│   │
│   ├── Virtual Fleet · orca-west
│   └── Virtual Fleet · orca-euc
│
├── Fleet · TITAN_2                          ★
├── Fleet · TITAN_QA                         ★
│
...
★ marks a fleet pinned to the sidebar; trailing numbers are counts (streams defined, alerts unacknowledged). **Streams is the only branch that descends further** — everything else at VF level is a leaf surface. Fleets a user's grants do not reach are absent from the tree entirely, not shown locked; `Browse all fleets…` opens the Global Fleet console rather than expanding in place.

Detailed surface specifications: `global-fleet-ui.md`, `fleet-ui.md`, `virtual-fleet-ui.md`, `stream-ui.md`, `connection.md`, `jobs-ui.md`, `events-ui.md`, `timeline.md`, `tables-ui.md`, `activation-ui.md`, `compare-ui.md`, `alerts-ui.md`, `users-permissions-ui.md`.

#### 2.2.2 The one scope rule

**A count shown at any level matches the contents of the screen it opens.** Every list — streams, connections, jobs, events, alerts — derives from a single scope predicate applied to one dataset, so no screen keeps a private copy or a differently-filtered tally. This is what makes the hierarchy navigable rather than merely nested: a number is a promise about the screen behind it.

#### 2.2.3 Four shared surfaces

Four components appear at multiple levels, each with one implementation and a widening scope. This is the console's main structural economy — and the reason behavior is identical wherever a surface appears.

**Logs — a four-level hierarchy.** `<stream>.out` → `<vf>.vf.out` → `<fleet>.fleet.out` → `global.out`. Each level is a strict superset of the one below, merged by time, with lines prefixed by the job (and, above stream level, the VF). One tabbed surface hosts them all: tabs persist across navigation, dock in the page or float as an overlay, and carry the same five filters (Capture · Integrate · Latency · Errors · Events) with per-tab state. Copy and Download emit the *filtered* view.

**Timelines — four scopes, two shapes.** Stream and VF scope render charts (bands, markers, one scrub cursor, a computed diagnosis); fleet scope renders VF cards; global renders the triage strip. The shift from charts to cards at fleet level is deliberate — combining metrics across independent virtual fleets would sum unrelated systems.

**Jobs and Events — one record, two tenses.** A job is an event that has not closed. Jobs is the open set (present tense, sorted by urgency, carries actions); Events is the closed set (past tense, immutable, sorted by time, carries evidence). A finished job does not *write* an event — it **becomes** one, keeping its id. The timeline is where the two tenses reconcile: both link into it at their own timestamp.

**Triage — the same ranking at three levels.** Global (needs-triage strip, with a 90-day history window), fleet (VF cards split triage-first), VF (the CONCERNS band). Every triage row is a deep link that switches context and lands on the exact problem surface.

#### 2.2.4 Permissions are structural, not decorative

Grants are hierarchical and strictly downward-editing: **SuperAdmin / SysAdmin → Fleet Admin (HubOwner at fleet scope) → VF Admin (HubOwner at VF scope) → ReadExec → ReadOnly**. An admin can never widen their own reach — a Fleet Admin cannot grant repository scope, a VF Admin cannot grant fleet scope.

The console expresses this by **omission, not disablement**: an action the current user cannot perform is not rendered. There are no greyed-out buttons teasing capability, and objects a user's grants do not reach are absent from lists entirely rather than shown locked. Visibility follows grants even for SuperAdmins' subordinates. Specified in `fleet-hierarchy.md` and `users-permissions-ui.md`.

#### 2.2.5 Consistency rules the console holds to

These are cross-cutting invariants, enforced everywhere rather than per screen:

1. **Nothing mutates without a plan.** Every definition change — new stream, added tables, changed method — enters through a computed diff the operator approves. Re-activation of an unchanged definition reports "no change".
2. **Derived state, never asserted.** A stream's status comes from its job states; a triage row comes from a threshold crossing. Nothing is stored as a display value.
3. **Deterministic ordering.** Live metrics tick, but rows never reshuffle: thresholds evaluate against wobble-free values and sorts carry explicit tiebreaks.
4. **Absolute time.** Incidents sit at wall-clock times, so zooming a range never rewrites the story.
5. **Display names are aliases.** Renaming a fleet changes a label; enrollment ids, grant scopes, log file names, and event records are untouched.
6. **Absence is a state.** A job that is owed and missing is a red row, not a quiet gap — the console reconciles against what is expected, it does not enumerate what exists.
7. **Every action shows its equivalent CLI** and is exportable, per the API-parity rule in section 7.
8. **Every change is ledgered** with attribution, previous → new state, and an artifact.

#### 2.2.6 What the console deliberately does not do

It holds no business logic the API lacks, caches no authority decisions locally, and offers no path that bypasses the plan/apply discipline. Where a capability exists only in the UI, that is a bug in the API — not a feature of the console.
...
Framed records: length-prefixed protobuf frames, one per row change, carrying operation type, table identity, before/after images, source position (SCN/LSN/LRSN), and a **change-origin lineage**: an ordered list of (location, stream) entries recording every hop the change has traversed — hop-0, the original producer, first; each capture that reads replicated-in changes appends its own entry. The encoding keeps the per-record cost at a few bytes regardless of chain depth: the file envelope header carries an **origin dictionary** (the distinct (location, stream) pairs appearing in the file), and each record's lineage is a sequence of varint indexes into it. Chain depth is bounded (16 hops); a record arriving at the bound fails capture with a loop-suspected error — never silent truncation of history. The lineage ships in v1 because its consumers pull in different directions and all of them hang off the format: loopback detection (bidirectional) needs the most recent hop, cascading origin filters and audit tracing need the full ordered path, and n-way replication needs both — and retrofitting a shipped file format is a migration. The TimeKey `_origin` column (stream spec 3.1) exposes this same lineage to customers, hop-0 first.
...
3. **The parser filters first, decodes second.** The redo record's object ID is checked against the stream's enrollment snapshot; changes to unsubscribed tables are skipped before column decoding — they cost almost nothing and never leave the host.
4. **The change becomes a frame**: one protobuf record carrying operation type, table identity, before/after images, the SCN, transaction ID, commit timestamp, and the change-origin lineage — a single entry here (this stream, this location), because this is hop-0; in a cascade, each subsequent capture appends its own entry to the chain.
...
6. **The file ships to the hub over mTLS** and is written durably to the hub file store under `<stream>/<sequence>`. Only now does the capture agent advance its checkpoint past this SCN — a crash before this point means re-reading redo, never losing data.
...
9. **The merge applies the burst** — insert/update/delete against the target table per the stream's replication style — **and in the same transaction updates the state table** with this file's position and integrate sequence. That co-transactionality is the exactly-once guarantee in one sentence: the data and the record of having applied it commit together or not at all.
...
Enterprise flat license: unlimited sources, streams, rows, and hubs for one legal entity plus subsidiaries; zero metering, hence zero enforcement machinery beyond validating a signed license file at hub start and surfacing entitlement status in the Admin UI. Commercial model: perpetual license plus annual maintenance (updates, new connectors, support); if maintenance lapses, the software continues running forever at the last entitled version — replication never stops over a billing state. Grace behavior on expiry of a term-limited evaluation license: alerts and no new stream creation; running streams are never halted. Dependency policy: MIT and Apache-2.0 crates only, enforced by cargo-deny in CI; the Oracle Instant Client is redistributed per its license terms and the decision is documented.

## 7. Interfaces

The web UI is a pure client of the public REST API — no private endpoints, no backdoors; anything the UI does, a curl command can do. Its application architecture — the four levels, the shared surfaces, and the consistency rules — is mapped in section 2.2. The API ships an OpenAPI specification from which the reference documentation is generated, so docs cannot drift from behavior. The CLI is a standalone static binary speaking only the REST API: drop it on any machine, point it at the hub with a token — no product installation required, unlike HVR's remote CLI. Stream and location definitions are exportable/appliable as version-controlled configuration through the CLI, enabling GitOps workflows.
...
| B | End-to-end stream | ARC-01, ARC-05, ARC-07 | Integration lab, TPC-C | Phase A exit; hub + agents runnable | Compare-clean stream; parity sweep and repo audit green |
...
**Integration: the lab stream.** The Docker lab (PostgreSQL 17 seeded from the existing pg17-lab setup, later Oracle Free 23ai, SQL Server, DB2) runs HammerDB TPC-C to generate realistic change volume through complete streams. Every integration run ends with a compare job proving source equals target — the product verifies its own correctness, and that verification gates the merge.
...
### ARC-01 — End-to-end stream correctness
**Steps:** (1) Stand up the lab stream (Postgres → hub → Postgres). (2) Run HammerDB TPC-C for 30 minutes. (3) Drain; run compare across all stream tables.
...
**Preconditions:** Broadcast stream to targets T1, T2; ability to pause T2's integrate.
...
**Steps:** (1) After a full TPC-C benchmark run, dump the repository schema and row samples. (2) Run the automated audit asserting no table stores change payloads; grep dumps for seeded canary values that flowed through the stream.
...
**Steps:** (1) Under load, take a pg_dump of the repository. (2) Destroy the hub host (retain the file store volume). (3) Provision a fresh hub; restore the dump; attach the file store. (4) Verify all streams resume; drain; compare.
...
**Steps:** (1) Start a hub with a maintenance-expired (but perpetual) license under running streams; observe for 24 hours. (2) Attempt to create a new stream (per documented lapse behavior). (3) Attempt to upgrade the hub to a version post-dating entitlement.
...
**Evidence:** 24-hour stream continuity metrics, refusal messages, empty egress capture.
...
**Preconditions:** TimeKey stream into the lab file target (S3-compatible store); chaos harness; a never-killed control run of the identical source change stream for reference.
...
**Preconditions:** Broadcast stream with a configurable per-hub file-store quota, capture spool enabled and sized for the test, capacity metrics and alerts enabled, and a continuous TPC-C workload.
...
**Preconditions:** A supported prior hub release with populated repository metadata, active lab streams, a verified repository backup, and a target release containing at least one representative schema migration.
**Steps:** (1) Run upgrade preflight; archive its version, checksum, backup, capacity, and compatibility report. (2) Upgrade while streams are active; verify new dispatch stops, active work checkpoints, migrations apply under the upgrade lock, and streams resume. (3) Run a second upgrade with an injected migration failure; verify no job dispatch begins and the prior schema and binary remain usable. (4) Execute the documented rollback path from the verified backup to the prior release, resume streams, drain, and compare.
...
| ID | Criterion |
|---|---|
| ARC-01 | End-to-end stream (Postgres → file log → Postgres) delivers a TPC-C change stream with source-equals-target proven by compare |
...
| ARC-06 | Hub restore from pg_dump plus existing file log resumes all streams correctly |
| ARC-07 | Every UI operation is reproducible via documented REST calls (automated UI-vs-API parity sweep) |
| ARC-08 | Origin lineage present, complete, and correctly ordered on every record across a cascading two-hop stream: hop-2 records carry the two-entry chain whose first entry traces to hop-0 |
...
| ARC-14 | A normal repository migration, an injected migration failure, and rollback to a verified pre-upgrade backup preserve the declared schema state, prevent unsafe job dispatch, and converge active streams with zero source-to-target divergence |
