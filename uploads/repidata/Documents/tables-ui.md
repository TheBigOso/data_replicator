...
**Tables (list)** — every table across every pipeline in the virtual fleet, filterable to one pipeline. Reached from the sidebar under a stream, or from "view all in Tables" on a pipeline detail page (which pre-filters to that pipeline).

**Table detail** — one table, everything about it. Reached by **clicking the table name** in the list. There are no per-row Edit or Drop-column buttons in the list: structural editing belongs on the detail screen, where the operator can see the whole structure they are changing. The list keeps only Remove, which is a pipeline-membership act rather than a structural one.
...
| Pipeline | Owning pipeline |
| Name in source | Physical name with schema prefix |
| Group | Table group |
...
Controls: search across identities and physical names, a pipeline filter, and Add table. The fixture spans seven pipelines and eight groups (FINANCE, PROCURE, SUPPLY, SHOPFLOOR, PEOPLE, SALES, IOT), including a fully stopped pipeline (`mssql-hr`) so the dead-table case is visible rather than theoretical.

## 3. Table detail

### 3.1 Header and facts

Identity, replication state pill, group and pipeline; below it the name mapping — `MFG.WORKORDERS → lakehouse.mfg_raw.workorders`. Then a six-card facts strip: **Source** (dialect + physical name), **Target** (dialect + physical name), **Rows in source** (estimate from the last refresh), **Changes today** (with time since last change), **Capture** (method + the supplemental-logging mechanism that method requires), **Apply latency** (p50, end to end).
...
| --- | --- | --- |
| `colEdits` | `{ table: { originalColumnName: { name?, tname?, type?, ttype?, nullable?, key?, note? } } }` | Session |
| `droppedCols` | `{ table: { originalColumnName: true } }` | Session |
