...
The base case: one stream, one source, one target. Everything in the architecture document describes this path.
...
| Consolidation | Yes | v1, per-stream state isolation |
...
### TOP-02 — Consolidation isolation under stream failure
**Preconditions:** Three TPC-C sources (S1–S3) consolidating into one Postgres target with per-stream state tables.
...
**Expected:** Hop-1 records carry a single lineage entry identifying stream S→M; hop-2 records carry the ordered two-entry chain — the original S→M entry first, the M→T entry appended — per the format spec; end-to-end compare clean.
...
**Steps:** (1) Configure stream 1 (succeeds). (2) Attempt stream 2 with the colliding table and no rename mapping. (3) Add a rename mapping and retry.
**Expected:** Step 2 is refused at configuration time with a message naming both streams and the colliding object; step 3 succeeds; no job ever launched in the failing case.
**Evidence:** Refusal message capture, event log showing zero job starts.

### TOP-05 — Bidirectional loopback soak (v2)
**Preconditions:** Postgres pair A↔B in bidirectional stream; bilateral write generators (disjoint key ranges); origin-marker accounting tool.
...
**Steps:** (1) Drive the topology view for each arrangement. (2) Assert every location, stream, and direction is present and correctly connected in the rendered DOM. (3) Capture screenshots (feeding the docs stream).
...
| TOP-02 | Consolidation of three sources into one Postgres target: state tables remain isolated per stream; kill and resume one stream without disturbing the other two |
